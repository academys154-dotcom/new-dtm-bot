...
class Database:
    ...
    def reset_redeem_code_usage(self, code: str, tg_user_id: int) -> None:
        code = normalize_code(code)
        with closing(self.connect()) as conn:
            conn.execute(
                "UPDATE redeem_codes SET is_used = 0, used_by_tg_user_id = NULL, used_at = NULL WHERE code = ? AND used_by_tg_user_id = ?",
                (code, tg_user_id),
            )
            conn.commit()
...
def build_exam_preview(data: dict, instructions: str = "") -> str:
    ...
    return (
        ...
        f"PDF: {escape_html_text(data.get('exam_pdf_file_name') or '-')}\n"
...
def format_exam_file_caption(row: sqlite3.Row, code: Optional[str] = None) -> str:
    # Telegram document caption limiti kichik. Shuning uchun PDF captioni ataylab qisqa qilindi;
    # to'liq test ma'lumoti PDFdan keyingi alohida xabarda yuboriladi.
    title = compact_spaces(row["title"] or "Test")
    if len(title) > 120:
        title = title[:117] + "..."
    parts = [f"📄 <b>{escape_html_text(title)}</b>"]
    if code:
        parts.append(f"Kod: <code>{escape_html_text(code)}</code>")
    return "\n".join(parts)[:900]
...
async def send_exam_document(message: Message, exam_row: sqlite3.Row, code_text: str, reply_markup: Optional[ReplyKeyboardMarkup] = None) -> bool:
    pdf_file_id = (exam_row["pdf_file_id"] or "").strip()
    if not pdf_file_id:
        await tracked_answer(message, "❌ Bu testga PDF file biriktirilmagan. Admin testni qayta yuklashi kerak.", reply_markup=reply_markup)
        return False

    try:
        sent = await message.answer_document(document=pdf_file_id, caption=format_exam_file_caption(exam_row, code_text))
        DB.add_message_history(message.from_user.id, message.chat.id, sent.message_id, "bot")
    except TelegramBadRequest as exc:
        logger.exception("PDF yuborishda TelegramBadRequest: %s", exc)
        await tracked_answer(
            message,
            "❌ PDF faylni yuborishda xatolik chiqdi. Kod saqlanib qoladi, admin PDFni qayta yuklashi kerak bo'lishi mumkin.",
            reply_markup=reply_markup,
        )
        return False
    except Exception as exc:
        logger.exception("PDF yuborishda kutilmagan xatolik: %s", exc)
        await tracked_answer(
            message,
            "❌ PDF faylni yuborishda kutilmagan xatolik chiqdi. Kod saqlanib qoladi, qayta urinib ko'ring.",
            reply_markup=reply_markup,
        )
        return False

    try:
        await send_long_text(message, format_exam_caption(exam_row, code_text), reply_markup=reply_markup)
    except Exception as exc:
        # PDF yuborilgan bo'lsa, kodni bekor qilmaymiz. Faqat ma'lumot xabari yuborilmay qolgan bo'lishi mumkin.
        logger.exception("PDFdan keyingi ma'lumot xabarini yuborishda xatolik: %s", exc)
    return True
...
async def admin_exam_title(message: Message, state: FSMContext) -> None:
    ...
    if len(title) < 3:
        ...
        return
    await state.update_data(exam_title=title, exam_grade="ALL")
...
async def admin_exam_pdf(message: Message, state: FSMContext) -> None:
    ...
    if not is_pdf_document(message):
        ...
        return
    await state.update_data(exam_pdf_file_id=message.document.file_id, exam_pdf_file_name=message.document.file_name or "test.pdf")
    await state.set_state(AdminStates.waiting_exam_answer_key)
    await tracked_answer(message, f"✅ PDF qabul qilindi: {escape_html_text(message.document.file_name or 'test.pdf')}\n\nJavoblar kalitini yuboring. Masalan: ABCDABCD yoki A B C D yoki 1-A, 2-B", reply_markup=admin_flow_keyboard())
...
async def admin_exam_confirm(message: Message, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    required = ["exam_title", "exam_category", "exam_subject", "exam_duration", "exam_pdf_file_id", "exam_answer_key"]
...
async def user_redeem_code_handler(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    if redeemed:
        delivered = await send_exam_document(message, redeemed, code, reply_markup=main_menu(message.from_user.id))
        if not delivered:
            DB.reset_redeem_code_usage(code, message.from_user.id)
            await tracked_answer(message, "🔁 Kod qayta tiklandi. PDF muammosi tuzatilgach, shu kodni yana ishlatishingiz mumkin.", reply_markup=main_menu(message.from_user.id))
    else:
        await tracked_answer(message, msg, reply_markup=main_menu(message.from_user.id))
...
async def text_router(message: Message, state: FSMContext) -> None:
    ...
    if text == BTN_FREE_TEST:
        ...
        delivered = await send_exam_document(message, exam, gift_code, reply_markup=subject_action_keyboard(message.from_user.id, grade_for_demo, subject))
        if delivered:
